...
//                String realFileName = System.currentTimeMillis() + fileName.substring(fileName.indexOf("_") + 1);
    ...
    @ApiOperation(value = "列出S3桶内的所有对象")
    public AjaxResult testS3ooooo(String bucketname) {
        ObjectListing objectListing = is3Service.listObjects(bucketname);
        return  AjaxResult.success(objectListing);
...
//                    //上传压缩后的图片
                ...
                //上传图片
                // is3Service.putObject(bucketname, key, yasuofileio);
                is3Service.putObject(bucketname, key, fileio);
                //删除原来的图片
                if(null!=fileio){
                    if (!fileio.delete()) {
                        log.info("删除成功！");
...
//                    importFileInfo.setCompressFileKey(keynew);
     ...
     * 单个图片文件通用上传请求(图片压缩)
     * @param file 文件
     * @param businessKey 属于那个业务
     * @param scale   图片大小（长宽）压缩比例 从0-1，1表示原图
     * @param outputQuality 图片质量压缩比例 从0-1，越接近1质量越好
     * @return
     */
    @Transactional
    @PostMapping("/compress/Img")
    @ApiOperation(value = "S3上传图片压缩")
    public AjaxResult uploadCompressFileImg(MultipartFile file, String businessKey,Float scale, Float outputQuality) {
        AjaxResult ajax = AjaxResult.success();
        if(scale==null){
            scale=0.8f;
        }
...
//                    //上传压缩后的图片
     ...
     */
    @GetMapping("/enlargeFileImg")
    @ApiOperation(value = "根据压缩过后的文件Key回显给前端进行图片放大")
     ...
     * 图片回显前端
     */
    @GetMapping(value = "/s3Image-Echo")
    @ApiOperation("S3图片回显前端")
...
//                String extension = mimeType.getExtension();
     ...
     * @param zipFileName
     */
    @GetMapping(value = "/s3compressZip")
    @ApiOperation("S3多文件进行压缩下载")

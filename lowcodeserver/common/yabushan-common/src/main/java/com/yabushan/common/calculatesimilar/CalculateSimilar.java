package com.yabushan.common.calculatesimilar;
...
public class CalculateSimilar {
    ...
    public static Result getSimilarResult(Reader reader, Reader reader1) throws IOException {
        ...
        //构造比较文本的特征向量
        for (String s : words1){
            if(fre.get(s)!=null && fre1.get(s)!=null){
                text.add(s);
            }
        ...
        double  B = Math.sqrt(sumB);
        BigDecimal AB = BigDecimal.valueOf(A).multiply(BigDecimal.valueOf(B));
        double v = BigDecimal.valueOf(sumAB).divide(AB, 3, BigDecimal.ROUND_HALF_UP).doubleValue();
        result.setSim(String.valueOf(v));
        result.setText(text);
        return  result;
    }
}

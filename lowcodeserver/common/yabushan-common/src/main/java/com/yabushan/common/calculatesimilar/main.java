...
public class main {
     ...
     * @throws IOException
     */
    public static Result getAnalysisResultInfo(String s1, String s2) throws IOException {
        //输出的单位：B字节
        //System.out.println(f.length());
        Result similarResult= new Result();
        ...
        try{
            ...
            if (f.length()!=0 && f1.length()!=0){
                if (f.length()<=300000){
                    similarResult= CalculateSimilar.getSimilarResult(reader, reader1);
                    // double sim = 100* CalculateSimilar.getSimilar(reader,reader1);
                   // System.out.println(">>>>"+sim);
                   // DecimalFormat df = new DecimalFormat();
                   // String s = df.format(sim);
                   // result.setSim(s);
                    ...
                    similarResult = SimHash.ConstructResult(reader, reader1);
                  /*  double sim = SimHash.Construct(reader,reader1);
                    DecimalFormat df = new DecimalFormat();
                    String s = df.format(sim);
                    result.setSim(s);*/

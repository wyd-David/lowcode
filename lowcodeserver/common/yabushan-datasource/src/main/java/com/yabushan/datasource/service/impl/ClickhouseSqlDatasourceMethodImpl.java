...
@Service
public class ClickhouseSqlDatasourceMethodImpl implements DatasourceMethodFactory {
    
    @Autowired
    private ClickhouseSqlEngine clickhouseSqlEngine;
    @Autowired
    ...
    public boolean validataDatasource(String driverClass, String ip,String port,String database, String userName, String password) throws Exception {
        ...
        String url ="jdbc:clickhouse://"+ip+":"+port+"/"+database;
        conn = DBUtil.getConnection(driverClass, url, userName, password);
        boolean execute = clickhouseSqlEngine.execute(conn, "select 1 ");
        return  execute;
    }
    ...
    public boolean getDataSourceMetaData(String datasourceId,List<String> tableNameList) throws Exception {
        ...
        String sql="SELECT name as TABLE_NAME,comment as TABLE_COMMENT  FROM system.tables  WHERE database ='"+appUnitInstance.getInstanceCode()+"'";
        StringBuffer buffer = new StringBuffer();
        if(tableNameList!=null && tableNameList.size()>0){
            buffer.append(" and name in('");
        ...
        clickhouseSqlEngine.getDatasourceAndFiled(conn, sql,userName,datasourceId);
        return true;
    }
    ...
    public DataSource getOneDatasourceFromPool(String datasourceId) throws Exception {
        ...
        String url ="jdbc:clickhouse://"+ip+":"+port+"/"+appUnitInstance.getInstanceName();
        ...
        metadataDatasourceTypeParam.setDbTypeId("clickhouse");

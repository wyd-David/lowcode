 ...
 */
@Service
public class Db2SqlEngine extends DynamicDataSourceFactory implements  DBSqlEngine{
    ...
    public void getDatasourceAndFiled(Connection conn, String sql,String schema,String datasourceId) throws Exception {
        ...
        try{
            ...
            while (resultSet.next()) {
                ...
                //获取字段信息
                sql = " SELECT F.COLNAME  AS COLUMN_NAME,F.NULLS  AS IS_NULLABLE,F.TYPENAME  AS DATA_TYPE,F.LENGTH  AS CHARACTER_MAXIMUM_LENGTH, F.SCALE  AS NUMERIC_PRECISION,'' as EXTRA,F.DEFAULT  AS COLUMN_DEFAULT,F.TYPENAME  AS COLUMN_TYPE,F.REMARKS  AS COLUMN_COMMENT,F.KEYSEQ  AS COLUMN_KEY FROM SYSCAT.COLUMNS F  WHERE F.TABSCHEMA ='"+schema+"'";
                StringBuffer fieldBuf = new StringBuffer();
                fieldBuf.append(" AND F.TABNAME ='").append(TABLE_NAME).append("'");

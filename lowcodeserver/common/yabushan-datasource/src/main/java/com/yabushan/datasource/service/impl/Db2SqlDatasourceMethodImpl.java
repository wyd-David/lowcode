...
@Service
public class Db2SqlDatasourceMethodImpl implements DatasourceMethodFactory {
    
    @Autowired
    private Db2SqlEngine db2SqlEngine;
    @Autowired
    ...
    public boolean validataDatasource(String driverClass, String ip,String port,String database, String userName, String password) throws Exception {
        ...
        String url ="jdbc:db2://"+ip+":"+port+"/"+database;
        conn = DBUtil.getConnection(driverClass, url, userName, password);
        boolean execute = db2SqlEngine.execute(conn, "select 1 from SYSIBM.dual");
        return  execute;
    }
    ...
    public boolean registerDatasource( String userName, String passWord,String instanceName,String versionCode,String IP,String PORT,String instanceDisplayName,String Schema,String instanceId) throws Exception {
       //1.对账号进行加密
        ...
        appUnitInstance.setAppClassCode(Constants.DB2);
    ...
    public boolean getDataSourceMetaData(String datasourceId,List<String> tableNameList) throws Exception {
        ...
        //3.获取到所有表
        String userName = appUnitInstance.getInstanceCode().toUpperCase();
        ...
        String sql="SELECT  f.TABNAME  AS TABLE_NAME , f.REMARKS  AS  TABLE_COMMENT FROM SYSCAT.TABLES F WHERE F.TABSCHEMA ='"+appUnitInstance.getInstanceCode().toUpperCase()+"' ";
        StringBuffer buffer = new StringBuffer();
        if(tableNameList!=null && tableNameList.size()>0){
            buffer.append(" AND f.TABNAME = in('");
        ...
        db2SqlEngine.getDatasourceAndFiled(conn, sql,userName,datasourceId);
        return true;
    }
    ...
    public DataSource getOneDatasourceFromPool(String datasourceId) throws Exception {
        ...
        String url ="jdbc:db2://"+ip+":"+port+"/"+appUnitInstance.getInstanceName();
        ...
        metadataDatasourceTypeParam.setDbTypeId("db2");
        ...
        DataSource OneDatasource = OracleSqlEngine.createAndGetOneDatasource(datasourceId, url, userName, password, metadataDatasourceTypeParams);

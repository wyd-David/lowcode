 ...
 */
@Service
public class PostGreSqlEngine extends DynamicDataSourceFactory implements  DBSqlEngine{
    ...
    public void getDatasourceAndFiled(Connection conn, String sql,String schema,String datasourceId) throws Exception {
        ...
        try{
            ...
            while (resultSet.next()) {
                ...
                //获取字段信息
                sql = "SELECT  col.COLUMN_NAME AS COLUMN_NAME,col.IS_NULLABLE ,col.DATA_TYPE,col.character_maximum_length as CHARACTER_MAXIMUM_LENGTH,col.NUMERIC_PRECISION,'' as EXTRA,col.column_default as COLUMN_DEFAULT,udt_name as COLUMN_TYPE, pgd.description AS COLUMN_COMMENT, (SELECT CASE WHEN column_name IS NULL THEN '0' else '1' END  FROM information_schema.constraint_column_usage ccu JOIN information_schema.table_constraints tc ON (ccu.constraint_name = tc.constraint_name)   WHERE ccu.table_schema=col.table_schema  and constraint_type = 'PRIMARY KEY' AND ccu.table_name =col.table_name and ccu.column_name=col.COLUMN_NAME limit 1) as COLUMN_KEY FROM information_schema.columns AS col LEFT JOIN pg_catalog.pg_description AS pgd ON (col.table_name::regclass = pgd.objoid AND col.ordinal_position = pgd.objsubid) WHERE col.table_schema='"+schema+"'";
                StringBuffer fieldBuf = new StringBuffer();
                fieldBuf.append(" and  TABLE_NAME ='").append(TABLE_NAME).append("'");
                ...
                while (filedSet.next()) {
                    ...
                    if(StringUtils.isNotNull(COLUMN_KEY)){

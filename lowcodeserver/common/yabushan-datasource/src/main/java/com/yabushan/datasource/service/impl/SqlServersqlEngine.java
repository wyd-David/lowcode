 ...
 */
@Service
public class SqlServersqlEngine extends DynamicDataSourceFactory implements  DBSqlEngine{
    ...
    public void getDatasourceAndFiled(Connection conn, String sql,String schema,String datasourceId) throws Exception {
        ...
        try{
            ...
            while (resultSet.next()) {
                ...
                //获取字段信息
                sql = "SELECT s.TABLE_NAME ,COLUMN_NAME,IS_NULLABLE,DATA_TYPE,CHARACTER_MAXIMUM_LENGTH,NUMERIC_PRECISION,'' AS EXTRA,COLUMN_DEFAULT,DATA_TYPE as COLUMN_TYPE,(select k.type from sys.tables t , sys.key_constraints k where k.object_id =t.object_id and t.type='U' and t.name =s.TABLE_NAME  ) as COLUMN_KEY,(SELECT ep.value FROM sys.tables t INNER JOIN sys.columns c ON t.object_id = c.object_id LEFT OUTER JOIN sys.extended_properties ep ON ep.major_id = t.object_id AND ep.minor_id = c.column_id WHERE t.name =s.TABLE_NAME and c.name=s.COLUMN_NAME) as COLUMN_COMMENT FROM INFORMATION_SCHEMA.COLUMNS s where s.TABLE_CATALOG ='"+schema+"'";
                StringBuffer fieldBuf = new StringBuffer();
                fieldBuf.append(" and  s.TABLE_NAME ='").append(TABLE_NAME).append("'");

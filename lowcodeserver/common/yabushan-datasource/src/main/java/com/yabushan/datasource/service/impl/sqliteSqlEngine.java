 ...
 */
@Service
public class sqliteSqlEngine extends DynamicDataSourceFactory implements  DBSqlEngine{
        ...
        try{
            ...
            while (resultSet.next()) {
                ...
                while (filedSet.next()) {
                    ...
                    String COLUMN_NAME = filedSet.getString("name");
                    ...
                    //是否为空
                    String IS_NULLABLE = filedSet.getString("notnull");
                    dataStructuredColumn.setIsNullable(IS_NULLABLE);


                    //数据类型 varchar
                    String DATA_TYPE = filedSet.getString("type");
                    dataStructuredColumn.setTypeName(DATA_TYPE);

                    //最大长度 CHARACTER_MAXIMUM_LENGTH
                    if(DATA_TYPE.contains("(") && DATA_TYPE.length()>0){
                        int i = DATA_TYPE.indexOf("(")+1;
                        String substring = DATA_TYPE.substring(i);
                        substring=substring.replace(")","");
                        //最大长度
                        dataStructuredColumn.setLength(Long.valueOf(substring));
                        //精度
                        dataStructuredColumn.setPrecision(substring);
                    }
                    //是否主键
                    String COLUMN_KEY = filedSet.getString("pk");
                    dataStructuredColumn.setIsPrimaryKey(COLUMN_KEY);

                    //默认值
                    String COLUMN_DEFAULT = filedSet.getString("dflt_value");
                    dataStructuredColumn.setDefaultValue(COLUMN_DEFAULT);

                    //中文名
                    String COLUMN_COMMENT = filedSet.getString("name");
                    dataStructuredColumn.setDisplayName(StringUtils.isEmpty(COLUMN_COMMENT)?COLUMN_NAME:COLUMN_COMMENT);

                    String COLUMN_TYPE = filedSet.getString("type");

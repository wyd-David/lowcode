  ...
  <div class="app-container">
    ...
    <div class="search-folder mb12" :class="{ 'folder-open': isOpen }">
      <expand-collapse>
        <div slot="left">
          <div
            ref="folder"
            :class="{ clearFormBottom: !isShowOpen }"
            style="flex: 1"
          >
            <el-form
              :model="queryParams"
    ...
    <div class="container">
      <el-row :gutter="10" class="mb12 utlsBtn">
        <el-col :span="1.5">
          <el-button
            ...
            size="mini"
            @click="handleAdd"
            v-hasPermi="['xuncha:groupLeader:add']"
            >新增
          </el-button>
        </el-col>
        <el-col :span="1.5">
          <el-button
            ...
            size="mini"
            @click="handleExport"
            v-hasPermi="['xuncha:groupLeader:importData']"
            >导出
          </el-button>
        </el-col>
        <!-- <right-toolbar
      ...
      >
        ...
        <el-table-column label="部门" align="center" prop="dpname" />
        ...
        >
          <template slot-scope="scope">
            <span>{{
              parseTime(scope.row.createdTime, "{y}-{m}-{d} {h}:{s}:{d}")
            }}</span>
          </template>
        </el-table-column>
        <el-table-column
        ...
        >
          <template slot-scope="scope">
            ...
            <el-button
              ...
              v-hasPermi="['xuncha:groupLeader:edit']"
              >修改
            </el-button>
            <el-button
              size="mini"
              ...
              v-hasPermi="['xuncha:groupLeader:remove']"
              >离库
            </el-button>
          </template>
        </el-table-column>
  ...
  data() {
    return {
      ...
      upload: {
        ...
        // 上传的地址
        url: process.env.VUE_APP_BASE_API + "/xuncha/groupLeader/importData",
      },
      // 查询参数
      queryParams: {
        pageNum: 1,
  ...
  methods: {
    ...
    getList() {
      ...
      listGroupLeader(this.queryParams).then((response) => {
        this.groupLeaderList = response.rows || [];
    ...
    handleUpdate(row) {
      ...
      getGroupLeader(id).then((response) => {
        this.form = response.data;
        this.open = true;
        this.title = "修改在库管理_组长管理";
      });
    },
    /**详情按钮*/
      ...
      this.$refs["form"].validate((valid) => {
        if (valid) {
          if (this.form.id != null) {
            updateGroupLeader(this.form).then((response) => {
            ...
            addGroupLeader(this.form).then((response) => {
              if (response.code === 200) {
    ...
    handleExport() {
      ...
      this.$confirm("是否确认导出所有在库管理_组长管理数据项?", "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
    ...
    /** 导入按钮操作 */
    openImportTable() {
      this.upload.title = "在库管理_组长管理数据导入";

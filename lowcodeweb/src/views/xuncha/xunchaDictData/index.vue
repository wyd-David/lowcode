<template>
  <div class="app-container">
    <expand-collapse>
      <div slot="left">
        <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="120px">
          <el-form-item label="字典类型ID：" prop="dictId">
            <el-input
    ...
    <div class="container">
      <el-row :gutter="10" class="mb12 utlsBtn">
        <el-col :span="1.5">
          <el-button
            ...
            size="mini"
            @click="handleAdd"
            v-hasPermi="['xuncha:xunchaDictData:add']"
          >新增
          </el-button>
        </el-col>
        <el-col :span="1.5">
          <el-button
            ...
            :disabled="single"
            @click="handleUpdate"
            v-hasPermi="['xuncha:xunchaDictData:edit']"
          >修改
          </el-button>
        </el-col>
        <el-col :span="1.5">
          <el-button
            ...
            :disabled="multiple"
            @click="handleDelete"
            v-hasPermi="['xuncha:xunchaDictData:remove']"
          >删除
          </el-button>
        </el-col>
        <el-col :span="1.5">
        ...
        <el-col :span="1.5">
          <el-button
            ...
            v-hasPermi="['xuncha:xunchaDictData:importData']"
          >导入
          </el-button>
        </el-col>
        <el-col :span="1.5">
      ...
      <el-table v-loading="loading" :data="xunchaDictDataList" @selection-change="handleSelectionChange">
        <el-table-column type="selection" width="55" align="left"/>
        <el-table-column label="字典项ID" align="center" prop="dataId"/>
        <el-table-column label="字典类型ID" align="center" prop="dictId"/>
        <el-table-column label="字典项编号" align="center" prop="dataCode"/>
        <el-table-column label="字典项名称" align="center" prop="dataName"/>
    ...
    <el-dialog :title="title" :visible.sync="open" width="600px" append-to-body :close-on-click-modal="false">
      <el-form ref="form" :model="form" :rules="rules" label-width="120px" :disabled="disabled">
        <el-form-item label="字典项编号：" prop="dataCode">
          <el-input v-model="form.dataCode" placeholder="请输入字典项编号"/>
        </el-form-item>
        <el-form-item label="字典项名称：" prop="dataName">
          <el-input v-model="form.dataName" placeholder="请输入字典项名称"/>
        </el-form-item>
      </el-form>
...
<script>
  ...
  import { addXunchaDictData, delXunchaDictData, disableEnable, exportXunchaDictData, getXunchaDictData, importTemplate, listXunchaDictData, updateXunchaDictData } from '@/api/xuncha/xunchaDictData'
  ...
  export default {
    ...
    data() {
      return {
        ...
        upload: {
          ...
          // 上传的地址
          url: process.env.VUE_APP_BASE_API + '/xuncha/xunchaDictData/importData'
        },
        // 查询参数
        queryParams: {
          pageNum: 1,
    ...
    methods: {
      ...
      getList() {
        this.loading = true
        listXunchaDictData(this.queryParams).then(response => {
          this.xunchaDictDataList = response.rows || []
      ...
      handleAdd() {
        this.disabled = false
        this.reset()
        this.form.dictId = this.$route.params && this.$route.params.dictId
        this.open = true
        this.title = '添加字典数据 '
      },
      /** 修改按钮操作 */
        ...
        this.$refs['form'].validate(valid => {
          if (valid) {
            if (this.form.dataId != null) {
              updateXunchaDictData(this.form).then(response => {
              ...
              addXunchaDictData(this.form).then(response => {
                if (response.code === 200) {
      ...
      handleDelete(row) {
        ...
        this.$confirm('是否确认删除字典数据 编号为"' + dataIds + '"的数据项?', '警告', {
      ...
      handleExport() {
        ...
        this.$confirm('是否确认导出所有字典数据 数据项?', '警告', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(function() {
          return exportXunchaDictData(queryParams)
        }).then(response => {

<template>
 ...
 -->
        ...
        <el-dialog :title="upload.title" :visible.sync="upload.open" width="400px" append-to-body>
            <el-upload ref="upload" :limit="1" accept=".xlsx, .xls" :headers="upload.headers" :action="upload.url + '?updateSupport=' + upload.updateSupport + '&formId='+upload.formId+'&listId='+upload.listId" :disabled="upload.isUploading" :on-progress="handleFileUploadProgress" :on-success="handleFileSuccess" :auto-upload="false" drag>
        ...
        <div class="container" v-if="this.childFormId">
             <el-table
                :data="childTableData"
                stripe
                resizable
              ...
              >
                ...
                <el-table-column
                  v-for="(item, index) in tableHeadChild"
                  :key="index"
                  :prop="item.prop"
                  :label="item.label"
                  :width="item.width"
...
<script>
    ...
    export default {
        ...
        mounted() {
          ...
          if(geturl.includes("&child")){
            //有子列表
             let getChildformId = getqys[2].split('=')[1]
             this.childFormId = getChildformId
          }
          ...
          getList(getlistid).then(response => {
            console.log(">>>>>>>>>",response.data);
                   ...
                   // let obj = JSON.parse(this.form.listMemo);
                    ...
                    this.tableHeadChild=this.form.listChildTitleVo;
           ...
           indexMethod(index) {
            ...
            getListChild(row) {
              if(this.childFormId!=null){
                this.loading = true;
                this.queryParamsChild.param =row;
                ...
                selectChildAutoTableData(this.queryParamsChild).then(response => {
                    ...
                    this.loading = false;
                    this.childTotal = Number(response.data.total);
                });
              }
            },
            ...
            handleAdd() {
                ...
                if(that.childFormId==null){
                  ...
                  //跳转到主子表单
                  const baseUrl = location.protocol+'//' + location.host+'/#/lowcodeFormChild?id='+this.formId+'&key=null&flag=2&listId='+ this.listid+"&childFormId="+this.childFormId+'&uuid='+Math.ceil(Math.random()*1000);
                  var flag = window.open(baseUrl);
                  var loop = setInterval(function(){
                      if(flag.closed){
                          clearInterval(loop);
            ...
            handleUpdate(row) {
              ...
              if(that.childFormId==null){
                ...
                //跳转子表
               // const baseUrl = location.protocol+'//' + location.host+'/#/lowcodeForm?id='+this.formId+'&key='+keyid+'&flag=3&listId='+this.listid+'&uuid='+Math.ceil(Math.random()*1000);
                const baseUrl = location.protocol+'//' + location.host+'/#/lowcodeFormChild?id='+this.formId+'&key='+keyid+'&flag=2&listId='+ this.listid+"&childFormId="+this.childFormId+'&uuid='+Math.ceil(Math.random()*1000);
                var flag = window.open(baseUrl);
                var loop = setInterval(function(){
                    if(flag.closed){
                        clearInterval(loop);
            ...
            toDetail(row) {
               if(this.childFormId==null){
                 ...
                 if(this.needViewBtn==='Y'){
                   ...
                   const baseUrl = location.protocol+'//' + location.host+'/#/lowcodeFormChildView?id='+this.formId+'&key='+keyid+'&flag=view&listId='+ this.listid+"&childFormId="+this.childFormId+'&uuid='+Math.ceil(Math.random()*1000);
            ...
            submitFileForm() {
              ...
              let getlistid =  getqys[0].split('=')[1];
              this.upload.listId=getlistid;
              console.log("this.upload.listId>>>>",this.upload.listId);

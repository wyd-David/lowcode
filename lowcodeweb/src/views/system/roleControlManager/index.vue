  ...
  <div class="app-container">
    ...
    <div class="container">
      <el-row :gutter="10" class="mb12 utlsBtn">
        ...
        <el-col :span="1.5">
          <el-button
            ...
            v-hasPermi="['system:control:importData']"
            >导入</el-button
          >
        </el-col>
        <el-col :span="1.5">
      ...
      >
        ...
        <router-link :to="'/roleControlManager/control/data/'+scope.row.roleControlId" class="link-type"> 
            <el-button
              size="mini"
...
export default {
  ...
  data() {
    return {
      ...
      upload: {
        ...
        // 上传的地址
        url: process.env.VUE_APP_BASE_API + "/system/control/importData",
      },
      // 查询参数
      queryParams: {
        pageNum: 1,
  ...
  methods: {
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map((item) => item.roleControlId);
    ...
    handleAdd() {
      ...
      this.reset();
      this.open = true;
      this.title = "添加 角色授权控制";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.disabled = false;
      this.reset();
      const roleControlId = row.roleControlId || this.ids;
      getControl(roleControlId).then((response) => {
        this.form = response.data;
        this.open = true;
        this.title = "修改 角色授权控制";
      });
    },
    /**详情按钮*/
    ...
    submitForm() {
      ...
      this.$refs["form"].validate((valid) => {
        if (valid) {
          if (this.form.roleControlId != null) {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const roleControlIds = row.roleControlId || this.ids;
      this.$confirm(
        '是否确认删除 角色授权控制编号为"' + roleControlIds + '"的数据项?',
        "警告",
        {
        ...
        .then(function () {
          return delControl(roleControlIds);
        })
        .then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      this.$confirm("是否确认导出 角色授权控制数据项?", "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })

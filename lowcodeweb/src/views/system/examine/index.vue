<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="examineList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="考核时间" align="center" prop="examineDate" width="180">
        <template slot-scope="scope">
          <span>{{ parseTime(scope.row.examineDate, '{y}-{m}-{d}') }}</span>
        </template>
      </el-table-column>
      <el-table-column label="考核综合得分" align="center" prop="examineScore" />
      <el-table-column label="考核等级" align="center" prop="examineLevel" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="600px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="考核时间" prop="examineDate">
          <el-date-picker clearable size="small" style="width: 200px"
        ...
        <el-form-item label="考核得分" prop="examineScore">
          <el-input v-model="form.examineScore" placeholder="请输入考核综合得分" />
        </el-form-item>
        <el-form-item label="考核等级" prop="examineLevel">
          <el-input v-model="form.examineLevel" placeholder="请输入考核等级" />
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加员工考核情况子集";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const recId = row.recId || this.ids
      getExamine(recId).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改员工考核情况子集";
      });
    },
    /** 提交按钮 */
    submitForm() {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除员工考核情况子集编号为"' + recIds + '"的数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return delExamine(recIds);
        }).then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出员工考核情况子集数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {

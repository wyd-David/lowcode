  ...
  <div class="app-container">
      ...
      <el-row :gutter="10" class="mb12 utlsBtn">
        <!-- <el-col :span="1.5">
          <el-button type="primary" icon="el-icon-plus" size="mini" @click="handleAdd"
            v-hasPermi="['sysJobLogBatch:systemJobLogBatch:add']">新增</el-button>
        </el-col>
        <el-col :span="1.5">
          <el-button type="primary" icon="el-icon-edit" size="mini" :disabled="single" @click="handleUpdate"
            v-hasPermi="['sysJobLogBatch:systemJobLogBatch:edit']">修改</el-button>
        </el-col>
        <el-col :span="1.5">
          <el-button type="danger" icon="el-icon-delete" size="mini" :disabled="multiple" @click="handleDelete"
            v-hasPermi="['sysJobLogBatch:systemJobLogBatch:remove']">删除</el-button>
        </el-col>
        <el-col :span="1.5">
          <el-button type="primary" icon="el-icon-lock" size="mini" :disabled="multiple" @click="disabledBtnClick(1)"
            v-hasPermi="['sysJobLogBatch:systemJobLogBatch:remove']">禁用</el-button>
        </el-col>
        <el-col :span="1.5">
          <el-button type="primary" icon="el-icon-unlock" size="mini" :disabled="multiple" @click="disabledBtnClick(0)"
            v-hasPermi="['sysJobLogBatch:systemJobLogBatch:remove']">启用</el-button>
        </el-col>
        <el-col :span="1.5">
          <el-button type="default" icon="el-icon-upload" size="mini" @click="openImportTable"
            v-hasPermi="['sysJobLogBatch:systemJobLogBatch:importData']">导入</el-button>
        </el-col>
        <el-col :span="1.5">
          <el-button type="default" icon="el-icon-download" size="mini" @click="handleExport"
            v-hasPermi="['sysJobLogBatch:systemJobLogBatch:importData']">导出</el-button>
        </el-col> -->
        <right-toolbar :showSearch.sync="showSearch" class="normalBtn" @queryTable="getList"></right-toolbar>
      </el-row>

      <el-table v-loading="loading" :data="systemJobLogBatchList" @selection-change="handleSelectionChange">
        <!-- <el-table-column type="selection" width="55" align="center" /> -->
        <el-table-column label="数据ID" align="center" prop="dataId"/>
        <el-table-column label="数据JSON字符串" align="center" prop="dataString" :show-overflow-tooltip="true"/>
        <el-table-column label="执行状态" align="center" prop="jobStatus" :formatter="jobStatusFormat" />
        <el-table-column label="异常信息" align="center" prop="jobExceptionInfo" :show-overflow-tooltip="true"/>
    ...
    <el-dialog :title="title" :visible.sync="open" width="600px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="120px" :disabled="disabled">
        ...
        <el-form-item label="批次ID" prop="dataBatchId">
          <el-input v-model="form.dataBatchId" placeholder="请输入批次ID" />
        </el-form-item>
        <el-form-item label="数据ID" prop="dataId">
          <el-input v-model="form.dataId" placeholder="请输入数据ID" />
        </el-form-item>
        <el-form-item label="数据JSON字符串" prop="dataString">
          <el-input v-model="form.dataString" placeholder="请输入数据JSON字符串" />
        </el-form-item>
        <el-form-item label="执行状态 0：成功   1：失败" prop="jobStatus">
          <el-select v-model="form.jobStatus" placeholder="请选择执行状态 0：成功   1：失败">
            <el-option v-for="dict in jobStatusOptions" :key="dict.dictValue" :label="dict.dictLabel"
...
<script>
  ...
  import {
    ...
    delSysJobLogBatch,
    addSysJobLogBatch,
    updateSysJobLogBatch,
    exportSysJobLogBatch,
    importTemplate,
    disableEnable
  } from "@/api/system/sysJobLogBatch/sysJobLogBatch";
  ...
  export default {
    ...
    data() {
      return {
        ...
        upload: {
          ...
          // 上传的地址
          url: process.env.VUE_APP_BASE_API + "/sysJobLogBatch/systemJobLogBatch/importData",
        },
        // 执行状态 0：成功   1：失败字典
        jobStatusOptions: [],
        // 查询参数
    ...
    methods: {
      ...
      getList() {
        ...
        this.queryParams.dataBatchId=id
        this.loading = true;
        listSysJobLogBatch(this.queryParams).then(response => {
          this.systemJobLogBatchList = response.rows;
      ...
      handleUpdate(row) {
        ...
        getSysJobLogBatch(id).then(response => {
        ...
        this.$refs["form"].validate(valid => {
          if (valid) {
            if (this.form.id != null) {
              updateSysJobLogBatch(this.form).then(response => {
              ...
              addSysJobLogBatch(this.form).then(response => {
                if (response.code === 200) {
      ...
      handleDelete(row) {
        ...
        this.$confirm('是否确认删除日志数据批次 编号为"' + ids + '"的数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return delSysJobLogBatch(ids);
        }).then(() => {
          this.getList();
      ...
      handleExport() {
        ...
        this.$confirm('是否确认导出日志数据批次 数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return exportSysJobLogBatch(queryParams);
        }).then(response => {

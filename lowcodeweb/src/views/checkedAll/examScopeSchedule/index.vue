<template>
    <div class="app-container">
            ...
            <div slot="left">
                <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="120px">
                    ...
                    <el-form-item label="被巡察单位：" prop="toExamCompanyName">
                        <el-input v-model="queryParams.toExamCompanyName" placeholder="请输入被巡察单位" clearable size="small"
                            @keyup.enter.native="handleQuery" />
                    </el-form-item>
                    <el-form-item label="考核类型：" prop="examTypeId" v-if="typeId == 1">
                        <el-select v-model="queryParams.examTypeId" placeholder="请选择考核类型" clearable size="small">
                            <el-option v-for="dict in examTypeOptiontip" :key="dict.dictValue" :label="dict.dictLabel"
        ...
        <div class="container">
            <div class="crad">
                <div class="crad-header">
                    <div class="crad-title">
                        ...
                        <span>{{ typeId == 1 ? '巡察组考核' : '巡察员考核' }}</span>
                    </div>
                </div>
            </div>
            ...
            <el-table v-loading="loading" :data="examScopeScheduleList" @selection-change="handleSelectionChange">
                ...
                <el-table-column label="当前状态" align="center" prop="status">
                    <template slot-scope="scope">
                        {{ scope.row.status | filterAll(approveStatusOption) }}
                    </template>
                </el-table-column>
                <el-table-column label="操作" align="center" class-name="small-padding fixed-width" fixed="right"
                    width="100">
                    <template slot-scope="scope">
                        <el-button size="mini" type="text" @click="handleDetails(scope.row)" class="btmLine"
                            v-if="scope.row.status==0 && scope.row.examTypeId != 4">填写分数</el-button>
                        <el-button size="mini" type="text" @click="handleDetails(scope.row)" v-else>查看</el-button>
                    </template>
                </el-table-column>
            </el-table>
            <div class="pageBox">
        ...
        <el-dialog :title="title" :visible.sync="isOpenQueryLeader" width="1200px" append-to-body="append-to-body"
            ...
            <div class="crad">
                        ...
                        <el-row :gutter="10">
                            ...
                            <el-col :span="12">
                                <el-form-item label="被考核组：" prop="groupName">
                                    ...
                                    <el-button type="primary" :disabled="false" class="detail"
                                        @click="onGrounpDetail(form)">查看</el-button>
                                </el-form-item>
                            </el-col>
                            <el-col :span="12">
                                <el-form-item label="被巡察单位：" prop="toExamCompanyName">
                ...
                <div class="crad-content" v-if="form.examTypeId == 1">
                    <el-row :gutter="10" class="mb12 utlsBtn mar20">
                        <el-col :span="1.5">
                            <el-button type="primary" size="mini" @click="handleExport(form)">
                                导出评分表
                            </el-button>
                            <el-button type="primary" size="mini" @click="handFraction(form)"
                                v-if="form.status==0 && editStatus == false">
                                填写分数
                            </el-button>
                            <el-button type="primary" size="mini" @click="handFractionSubmit(form,form.examTypeId)"
                                v-else-if="form.status==0 && editStatus" :disabled="btndisabled">
                                提交分数
                            </el-button>
                        </el-col>
                    </el-row>
                    <el-form ref="form1" :model="form" :rules="rules" label-width="120px" :disabled="disabled">
                        <el-table v-loading="loading" :data="form.checkedInfoList"
                            @selection-change="handleSelectionChange" border key="table1">
                            ...
                            <el-table-column align="center" v-for="(item, index) in tableData" :key="index"
                                ...
                                <template slot-scope="scope">
                                    <span v-if="scope.row.status==0 && editStatus">
                                        <el-form-item label="" prop="scope" :disabled="true" class="specialForm" :rules='{
                                            ...
                                            <el-input v-model="scope.row.examList[index]['examResult']"
                                                placeholder="请输入" clearable min="0" :max="item.mixScope"
                                                @blur="inpBlur1(scope.row.examList, scope.$index)"
                                    ...
                                    <span v-if="scope.row.status==0 && editStatus">
                                        <el-form-item label="" prop="scope" class="specialForm" :rules='{
                                            ...
                                            <el-input v-model="scope.row.examResult" placeholder="请输入" clearable
                                                :disabled="scope.row.ifSubmit == '1'" min="0"
                                                :max="scope.row.examMaxScope"
                                                @blur="inpBlur2(scope.row.groupId, scope.$index)"
                                                onkeyup="value=value.replace(/[^-0-9.]/g,'')" />
                                        </el-form-item>
                                    </span>
                                    <span v-else>{{ scope.row.examResult ? scope.row.examResult : '--' }}</span>
                                </template>
                            </el-table-column>
                ...
                <div class="crad-content" v-if="form.examTypeId == 3">
                    ...
                    <el-form ref="form3" :model="form" label-width="120px" :disabled="disabled">
                        <el-table v-loading="loading" :data="form.examList" border show-summary
                            :summary-method="getSummaries" key="table3">
                            <el-table-column label="序号" align="center" type="index" width="55" />
                            ...
                            <el-table-column label="*有/无" align="center" prop="ifHave" width="200">
                                <template slot-scope="scope">
                                    ...
                                    <span v-else>{{ scope.row.ifHave == 0 ? '无' : '有' }}</span>
                                </template>
                            </el-table-column>
                                    ...
                                    <span v-if="editStatus">
                                        <el-form-item label="" prop="examRemark" class="specialForm" :rules='{
                                          ...
                                          validator: (rule, value, callback) => {
                                            ...
                                            <el-input v-model="scope.row.examRemark" placeholder="请输入" type="textarea"
                                                :rows="3" maxlength="500" show-word-limit />
                                        </el-form-item>
                                    </span>
                                    <span v-else>{{ scope.row.examRemark ? scope.row.examRemark : '--' }}</span>
                                </template>
                            </el-table-column>
                        </el-table>
                    </el-form>
                ...
                <div class="crad-content" v-if="form.examTypeId == 4">
                    ...
                    <el-form ref="form4" :model="form" label-width="120px">
                        <el-table v-loading="loading" :data="proTeamList" border :header-cell-style="headCellstyle"
                            :header-row-style="headsrowStyle" key="table4">
                            <!-- <el-table-column type="selection" v-if="form.status==0" width="55" align="center" /> -->
                            <el-table-column label="序号" align="center" type="index" width="55">
                            ...
                            <el-table-column label="考评总分" align="center">
                                <el-table-column label="" align="center" width="105" prop="numberAll">
                                    ...
                                    <template slot-scope="scope">
                                        {{ scope.row.allpoints | toFixfiler }}
                                    </template>
                                </el-table-column>
                            </el-table-column>
                            <el-table-column label="考评等级" align="center">
                                <el-table-column label="" align="center" width="105" prop="levelId">
                                    <template slot-scope="scope">
                                        {{ scope.row.leavelId | filterAllLevel(scoreLevelption,form.status) }}
                                    </template>
                                </el-table-column>
                            </el-table-column>
                        </el-table>
            ...
            <div slot="footer" class="dialog-footer">
                <el-button @click="isOpenQueryLeader = false;editStatus = false;">关 闭</el-button>
            </div>
        </el-dialog>
...
import {
    ...
    exportLeaderListProgress,
    exportPatrolProgressInfo,
    exportToexamProgressInfo,
    exportGroupMemberProgress,
    batchSubmitLeaderListToExam,
...
export default {
    ...
    components: { ExpandCollapse, PatrolGroup },
    data () {
        return {
            btndisabled: false,
            editStatus: false,
    ...
    filters: {
        ...
        filterAllLevel (val, optionArr, status) {
            console.log(val, "等级", status)
            if (status != '1') return "--";
            var arr = optionArr.filter((item) => item.dictValue === val);
            if (!arr.length) return "";
            return arr[0].dictLabel;
    ...
    created () {
        this.typeId = this.$route.name.indexOf('Pre') < 0 ? 1 : 2;
        ...
        // })
        this.approveStatusOption = [{ dictValue: '0', dictLabel: '评分中' }, { dictValue: '1', dictLabel: '已完成' }, { dictValue: '2', dictLabel: '已过期' },]
        this.getDicts("exam_type").then(res => {
            this.examTypeOption = res.data ? res.data : [];
            let ken = JSON.parse(JSON.stringify(res.data));
            ken.length = 3;
            this.examTypeOptiontip = ken;
        })
    ...
    methods: {
        ...
        handleSelectionChange (selection) {
            this.ids = selection.map(item => item.fromId + "_" + item.toUserId)
        ...
        handleExport (form) {
            ...
            console.log(typeId, '1领导小组，2巡察办，3被巡察部门,4组员')
            console.log(datas);
            //const queryParams = this.datas;
            this.$confirm('是否确认导出所有巡察考核评分记录 数据项?', "警告", {
                ...
                if (typeId == "1") {
                    return exportLeaderListProgress(datas);
                } else if (typeId == "2") {
                    return exportPatrolProgressInfo(datas)
                } else if (typeId == "3") {
                    return exportToexamProgressInfo(datas);
                } else if (typeId == "4") {
                    return exportGroupMemberProgress(datas)
                }
            }).then(response => {
        ...
        handFractionSubmit (form, eid) {
            ...
            if (eid != '3') {
                ...
                checkedInfoList = that.form.checkedInfoList;
                checkedInfoList.map(item => {
                    item.toExamUserId = item.toUserId;
                    item.applyId = form.applyId,
                        item.groupId = form.groupId
                })

                console.log(data)
                checkedInfoList.forEach((item, index) => {
                    if (list.filter(stm => stm.fromUserId == item.fromUserId) >= 0) {
                        list.push({
                            examDetail: [item],
                            batchId: that.form.batchId ? that.form.batchId : '',
                            todoId: '',
                            year: that.form.year ? that.form.year : '',
                ...
                form.Allpoints = form.allpoints;
                list = [{ ...that.form, ...obj }]
            }
            // console.log(data)
            // debugger
            // return
            this.$refs["form" + eid].validate(valid => {
                if (valid) {
                    this.$confirm('是否提交评分？', '提示', {
                        confirmButtonText: '确定',
                        cancelButtonText: '取消',
                    }).then(() => {
                        that.btndisabled = true
                        const loading = that.openLoading();
                        batchSubmitLeaderListToExam(JSON.stringify(list)).then(res => {
                    ...
                    this.$message.warning("评分不能为空或输入正确的评分")
                }
            });
            // for (let i = 0; i < array.length; i++) {
            //     array[i].examList.forEach(item => {
            //         if (item.examResult.length == 0) {
            //             this.$message.warning("请填写" + item.examContant)
            //             arrayType = false;
            //             return
            //         }
            //     })
        ...
        inpBlur1 (val, indx) {
            ...
            let Allpoints = 0;
            Allpoints = this.form.checkedInfoList[indx].examList.reduce((r, n) => {
            ...
            this.form.checkedInfoList[indx].allpoints = Allpoints.toFixed(2);
        },
        inpBlur2 (val, indx) {
            console.log(val, indx)
            let Allpoints = 0;
        ...
        getSummaries (param) {
            ...
            console.log(this.form.allpoints, 'result');
            console.log(num)

<template>
  <div class="app-container">
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
     ...
     <steps-form ref="stepsFormRef" :dialogVisible="stepsFormShow" :processDefId="pId" @close="stepsFormShow = false"></steps-form>
  ...
  methods: {
    ...
    handleAdd() {
     // window.open(this.ipPort+"/create?name=activiti&key=yabushan_key");
      window.open("http://www.nangs.vip:8083"+"/create?name=activiti&key=yabushan_key");
      /* this.reset();
      this.open = true;
      this.title = "添加工作流模型"; */
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      window.open("http://www.nangs.vip:8083"+"/modeler?modelId="+row.id);
      /* this.reset();
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认发布工作流模型编号为"' + ids + '"的数据项?', "警告", {
          ...
          window.open("http://www.nangs.vip:8083"+"/models/"+ ids+"/deployment");
        }).then(() => {
          this.getList();
    ...
    handleSteps(row) {
      // this.pId = row.pId;
      this.stepsFormShow=true;
       this.$nextTick(() => {
          this.$refs.stepsFormRef.getList(row.pId);
        });
      // pId

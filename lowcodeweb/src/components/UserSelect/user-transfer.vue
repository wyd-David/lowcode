  ...
  <el-dialog class="custom-center-dialog" :title="title" :visible.sync="visible" :width="multiple?'860px':'560px'" :close-on-click-modal="false" append-to-body>
    <div class="transfer-wrapper" :style="{height:multiple?'400px':'350px'}">
      <div class="transfer-left-wrapper">
        <el-input placeholder="请输入员工姓名或部门名称查找" v-model="keyword" @keyup.enter.native="onSearch">
          <el-button slot="append" icon="el-icon-search" @click="onSearch"></el-button>
        </el-input>
        <div style="margin-top: 16px;" v-if="!multiple">
          已选择人员：{{selected?selected.name:''}}
        </div>
        <div class="transfer-left-content">
          <el-scrollbar>
						<UserTree ref="tree" :key="timeKey" :props="props" :multiple="multiple" :leaf-checked="leafChecked" @node-click="onNodeClick"></UserTree>
          </el-scrollbar>
        </div>
      </div>
      ...
      <template v-if="multiple">
        ...
        <div class="transfer-right-wrapper">
          <div class="selected-title">
            可多选，已选择&nbsp;<span>{{selected.length}}</span>&nbsp;人
            ...
            <el-scrollbar>
              <ul class="selected-list">
                <template v-for="(item,i) in selected">
                  <li :key="i">
                    <div class="selected-label">{{item.name}}</div>
...
<script>
	import UserTree from '@/components/UserSelect/user-tree';
  export default {
		components: {
		  UserTree
		},
    ...
    methods: {
      ...
      onRemove(item, index) {
        ...
        let keys = this.selected.map(item => item[valKey]);
      },
      // 清空
      onClear() {
        this.selected = [];

...
export default {
  ...
  methods: {
    ...
    uploadStart(file){
      ...
      console.log(file)
       this.$emit(kebabCase('uploadStart'), file)
    },
    onFileAdded(file) {
  ...
  // 计算MD5
      // await this.computeMD5(file, this.options.chunkSize);
      // Bus.$emit('fileAdded');
    ...
    fileRemoved (file) {
      ...
      this.$emit(kebabCase('fileRemoved'), file,this.$refs.uploader.files,this.$refs.uploader.fileList)
    },
    ...
    async onFileSuccess(rootFile, file, message) {
      ...
      res.data.previewUrl+=encodeURIComponent(Base64.encode(res.data.downUrl))
      file.result=res.data
      this.$emit(kebabCase('fileSuccess'), file,this.$refs.uploader.files,this.$refs.uploader.fileList)
      console.log('合并完成');
    }
  }

<template>
  <div>
    ...
    <!-- 审核 -->
    <el-dialog :title="title" modal-append-to-body append-to-body :visible.sync="dialogVisible" :before-close="close"
               width="600px" @open="()=>{isNextUser&&init()}">
      <div class="dialog-context">
        <el-form v-loading="loading" label-width="140px" :model="subForm" :rules="afRules" ref="afRules">
          <el-form-item label="下一环节：" prop="nextStep">
            <!-- <el-select
              value-key="outLineId"
              style="width: 100%"
          ...
          <el-form-item label="下一步处理人：" prop="id">
            <el-select style="width: 100%" v-model="subForm.id" placeholder="请选择">
              <el-option v-for="(item, index) in nextUsers" :key="item.id" :label="item.name" :value="item.id">{{
                item.name
                }}
              </el-option>
            </el-select>
      ...
      init(arr) {
        ...
        request({
          ...
          if (res.code == 200) {
            ...
            this.subForm.id = this.nextUsers[0] ? this.nextUsers[0].id : '';
            this.loading = false;
          }
        });
      },
